...
import { allSysBOMetadata, sysBOAddressesMetadata, sysBOPrincipalAddressesMetadata } from '@manatos/shared';
...
describe('address metadata', () => {
  ...
  it('links principals to canonical addresses through relationship metadata', () => {
    ...
    expect(sysBOPrincipalAddressesMetadata.relationships?.principal?.references.objectKey).toBe('sys-principals');
    expect(sysBOPrincipalAddressesMetadata.relationships?.address?.references.objectKey).toBe('sys-addresses');
  });
});
