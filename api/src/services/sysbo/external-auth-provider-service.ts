...
export class SysBOExtAuthProviderService extends GenericSysBOService<SysBOExtAuthProvider> {
  ...
  async createProvider(
    ...
    const initialized = this.applyCreateDefaults(
      input as unknown as SysBOCreateInput<SysBOExtAuthProvider>,
    );
    const provider = parseProvider(initialized.provider);
    ...
    const enabled = initialized.enabled === true;
    ...
    return this.create(
      {
        ...
        callbackPath: providerCallbackPath(initialized.callbackPath, definition.callbackPath),
        ...(provider === SysBOExtAuthProviderType.Microsoft
          ? { tenant: normalizeMicrosoftTenant(initialized.tenant) }
          : {}),
      },
      actor,
    );
