...
export class SysBOLicenseService extends GenericSysBOService<SysBOLicense> {
  ...
  override async create(
    ...
    const principal = await this.store.sysPrincipals.getById(initialized.principalId);

    if (!principal) {
      throw new NotFoundError('SysBOPrincipal', initialized.principalId);
    }

    const platform = resolvePlatform(MANATOS_COMPANY, initialized.platformId);
    if (platform.id !== initialized.platformId) {
      throw new NotFoundError('Platform', initialized.platformId);
    }

    if (initialized.applicationId) {
      ...
      const application = await this.store.sysApplications.getById(initialized.applicationId);
      if (!application) {
        throw new NotFoundError('SysBOApplication', initialized.applicationId);
      }
    }
