...
export class SecretsEncryptionService {
  ...
  private requireKey(): Buffer {
    if (!this.key)
      throw new Error(
        "SECRETS_ENCRYPTION_KEY is required before encrypted application secrets can be stored or read. Generate it once with: node -e \"console.log(require('crypto').randomBytes(32).toString('base64'))\"",
      );
    return this.key;
  }
}
