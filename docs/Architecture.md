...
Metadata can carry decisions as expressions rather than hard-coded renderer branches. Every renderable semantic field is defined once in canonical `fieldDefinition`; `fieldDefinition.<key>.calculation` supplies a calculated value source without creating a second field model or renderer. `persisted: true` opts an authoritative calculated field into generic pre-commit materialization, while optional `triggeredBy` supports editable assisted calculations. UI metadata can independently make tab/action visibility, field editability and presentation decoration evaluator-backed. Persisted calculated state remains metadata-driven rather than caller-driven, so direct API/background creation follows the same calculation as the UI.

Expression source is parsed by the shared engine into a typed AST when context metadata is materialized. The runtime evaluator resolves variables through the ManatOS CTX lexical scope. The metadata-driven browser renderer receives the AST and performs dependency-aware reevaluation without reparsing the expression source. This keeps one expression contract usable by EJS rendering, browser reactivity, diagnostics and future clients. Entry pages keep `entryOriginal` (immutable normalized baseline) and `entry` (live working record), while collection-owning pages expose `entriesOriginal[]` and live `entries[]` collections. AST dependencies subscribe to CTX value changes, and calculated mutations re-enter the same CTX setter/event path as user mutations so cascades settle generically. `TraverseCtx(...)` remains the materialized-CTX hierarchy function. Canonical persisted hierarchy calculations use resolver-capable `TraverseEntity(...)`; SysPrincipals use it for `rootPrincipalId`, independent of list filtering/paging/navigation.
...
`CompanyInfo` owns Company-wide capabilities and a code-defined catalogue of enabled `SysPlatform` entries. The selected platform contributes its own SysBO capabilities and navigation entries on top of the Company baseline. The current implementation ships with protoCRM, but horizontal navigation and platform landing pages are catalogue-driven so additional platforms do not require a separate shell design.

Each platform can provide a compact identity icon, branding/hero artwork, descriptive copy, capability cards and presentation assets. The UI exposes the current platform through a compact top-header identity and the horizontal **Platform** entry; the latter automatically becomes the platform catalogue menu when multiple platforms are enabled. Platform-owned metadata/routes/views/assets are isolated under platform subfolders (currently `shared/src/platforms/protocrm/`, `ui/src/platforms/protocrm/`, `ui/views/pages/platforms/protocrm/`, `ui/public/assets/platforms/protocrm/` and `ui/public/css/platforms/protocrm.css`) so future platforms do not add product-specific branches to generic routers/templates.
...
Metadata-driven aggregate/hierarchy workspaces keep their authoritative live state in CTX. Create workflows may declare a browser-local checkpoint lifecycle; Principal Create Organization stores its incomplete working graph under a stable user/entity create key, while Edit Organization deliberately has no draft lifecycle. **Commit** is the explicit aggregate persistence boundary: the owner sends `entries[]` plus `entriesOriginal[]` to the generic aggregate endpoint, which resolves temporary `draft:*` identities and applies the requested creates/updates/deletes in one datastore transaction. A successful create commit removes compatible browser drafts before returning to the owner list; failures preserve working state. Child `recordQuick` and owner-managed full-entry saves remain local to the owner and never independently persist.

### Entity entry representation

Canonical entity metadata may declare `entry.name/type/description/status`; UI metadata may separately declare entry icon presentation. Lists, hierarchy visualizations and future lookup/card renderers resolve those semantics through one generic entry-representation contract. Formula values use the common evaluator and relationship projections use `relations.<relationshipKey>.<field>`. See `Entity-Metadata.md`.
...
For platform-owned entities, authorization may also be license scoped. `SysApplication` belongs to protoCRM: Admin has unrestricted access; non-Admin collection and record reads require a current effective protoCRM `SysLicense` reached through a linked `SysPrincipal`, and an optional `applicationId` restriction narrows visibility to that application. The current in-memory adapter filters materialized rows before client filtering/paging; future RDBMS adapters should push the same predicate into the database query. 5. **System Configuration** — Admin-only persisted runtime configuration; sensitive values are never returned as plaintext. 6. **Public UI** — anonymous-safe bootstrap/runtime projections used before sign-in. 7. **External Authentication** — Admin provider configuration and supported-provider metadata. 8. **External Authentication Credentials** — trusted Admin/BFF secret lifecycle; requires Admin Bearer authentication plus `x-internal-api-key`. 9. **Internal External Authentication Workflow** — UI/BFF-only OAuth credential-test mechanics, not a general client API.
...
### Complete `ctx.entities` registry

`ctx.entities` is the canonical object/entity metadata registry, not the navigation catalogue. UI bootstrap seeds it from `allManatOSObjectMetadata`, including internal contact objects and relationship entities that intentionally have no top-level Administration page. Exposure/navigation remains a separate concern and must never determine whether canonical metadata is visible to CTX/evaluator/debugging consumers.
