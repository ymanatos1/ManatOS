...
Each platform can provide branding/hero artwork, descriptive copy, capability cards and presentation assets. The UI exposes the current platform both through the top-header badge and the horizontal **Platform** entry. Platform-owned metadata/routes/views/assets are isolated under platform subfolders (currently `shared/src/platforms/protocrm/`, `ui/src/platforms/protocrm/`, `ui/views/pages/platforms/protocrm/`, `ui/public/assets/platforms/protocrm/` and `ui/public/css/platforms/protocrm.css`) so future platforms do not add product-specific branches to generic routers/templates.
