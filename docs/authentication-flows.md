...
Microsoft, Google, Facebook and GitHub are handled by the UI/Passport layer through the shared executable provider-adapter registry under `ui/src/auth/providers/`. Declarative provider definitions own labels/icons/scopes/callback/help; each adapter owns only the code that genuinely differs (Passport strategy construction and native profile normalization). Both normal sign-in and credential testing resolve the same adapter registry. Provider identity is resolved by provider + provider subject. Matching email alone never silently links an external identity to an existing account; ownership confirmation is required according to the linking flow.
