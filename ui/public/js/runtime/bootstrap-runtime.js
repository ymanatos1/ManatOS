(() => {
  ...
  /**
   * Refresh the session-owned authenticated-user CTX projection. Entry Save only
   * announces that the persisted current user changed; this bootstrap/current-user
   * owner performs the authoritative re-projection from the server session.
   */
  const refreshAuthenticatedUser = async () => {
    const response = await fetch('/runtime/current-user-context', {
      method: 'GET',
    ...
    const runtime = window.ManatOS?.ctx;
    if (!runtime?.replace || !payload || typeof payload !== 'object') return;
    runtime.replace('ctx.user', payload.user ?? null, {
      source: 'ui-bootstrap-current-user-refresh',
      triggerPath: 'ctx.user',
    });
  };

  document.addEventListener('manatos:form-saved', (event) => {
    const form =
      event.target instanceof Element ? event.target.closest('form[data-entity-key]') : null;
    if (!(form instanceof HTMLFormElement) || form.dataset.entityKey !== 'sys-users') return;
    const savedId = event.detail?.id;
    const currentUserId = window.ManatOS?.ctx?.get?.('ctx.user.fields.id.value');
    if (!savedId || String(savedId) !== String(currentUserId ?? '')) return;
    void refreshAuthenticatedUser().catch((error) =>
      console.error('[ManatOS] Current-user CTX refresh failed.', error),
    );
  });

  window.ManatOS?.ctx?.trackSubscriber?.('*', { kind: 'runtime', label: 'UI bootstrap' });
