(async () => {
  ...
  if (form) {
    ...
    /*
     * The baseline belongs to the fully initialized form, not merely to the DOM
     * state at the end of this script. Entity/component runtimes loaded later in
     * the shell may still synchronously apply provider callbacks, generated
     * values, compound-field state, etc. Capture after all parser-loaded scripts
     * have completed so those initialization writes remain clean.
     */
     ...
     *
     * This is still one baseline and one dirty rule; these flags only join the
     * two existing initialization lifecycles before that baseline is captured.
     */
    ...
    const captureWhenInitialized = () => {
      if (initialBaselineCaptured || !metadataInitialized || !pageScriptsInitialized) return;
...
(async () => {
  const { calculateEntryAggregatePolicy } =
    await import('/shared-runtime/entry-aggregate-policy.js');
  ...
  document.querySelectorAll('form[data-dirty-guard="true"]').forEach((form) => {
    ...
    const update = () => {
      ...
      if (pagePath && runtime?.replace) {
        ...
        if (pagePath === 'ctx.ui.level' || pagePath.startsWith('ctx.ui.level.level')) {
          // The entry surface has one canonical aggregate "blocked" state;
          // internalEditing/internalEditorCount implementation details.
          if (runtime.get?.(`${pagePath}.state.blocked`) !== internalEditing) {
            runtime.replace(`${pagePath}.state.blocked`, internalEditing, { source: 'page-state' });
          }
        } else {
      ...
      const aggregatePolicy = calculateEntryAggregatePolicy({
        mode: recordMode,
        fieldDirty: changed,
        fieldValid: valid && credentialStateAllowsSave,
        blocked: internalEditing,
      });
      const saveDisabled = !aggregatePolicy.saveReady;
