...
describe('popup infrastructure presentation', () => {
  ...
  it('keeps every popup explicit while leaving Developer Tools interactive above popup layers', async () => {
    const popupRuntime = await readFile(resolve(testDirectory, '../public/js/popups/popup-runtime.js'), 'utf8');
  ...
  it('projects Bootstrap popups into the same callingParams/presentation/state CTX contract as custom popups', async () => {
    const popupRuntime = await readFile(resolve(testDirectory, '../public/js/popups/popup-runtime.js'), 'utf8');
    const recordSelector = await readFile(resolve(testDirectory, '../public/js/popups/record-selector.js'), 'utf8');
    ...
    expect(popupRuntime).toContain("const popupPath = () => `${leafPagePath() || 'ctx.page'}.popup`");
