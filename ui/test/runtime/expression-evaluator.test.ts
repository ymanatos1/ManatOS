...
describe('ManatOS expression parser/evaluator', () => {
  ...
  it('derives the current V2 UI level and full active chain from nested ctx.ui.level contexts', () => {
    const entry = { id: 'entry', kind: 'entry', state: { dirty: false } };
    const popup = { id: 'popup', kind: 'selector', level: entry };
    const list = { id: 'list', kind: 'list', level: popup };
    const ctx = { ui: { level: list } };

    expect(evaluateTest('CurrentUiLevel()', ctx, entry)).toBe(entry);
    expect(evaluateTest('TraverseUiLevels()', ctx, entry)).toEqual([list, popup, entry]);
  });
