  ...
  it('hosts CTX Viewer and API Traffic as tabs of one dock and remembers the active tab', () => {
    ...
    expect(sourceWithoutWhitespace(debuggerCssSource)).toContain(
      sourceWithoutWhitespace(
        '.developer-tools-tab-caption { min-width: 0; overflow: hidden; color: #172b4d; font-weight: 400; text-overflow: ellipsis; white-space: nowrap; }',
      ),
    );
    expect(sourceWithoutWhitespace(debuggerCssSource)).toContain(
      sourceWithoutWhitespace(
    ...
    expect(shellSource).not.toContain('setApiTrafficVisible(');
