...
describe('generic related-collection presentation', () => {
  ...
  it('evaluates related-row calculations and presentation in the browser from canonical ASTs', () => {
    ...
    expect(runtime).toContain('expressionCompiler.ast(valueSource)');
    expect(runtime).toContain('expressions.evaluateAstOwnedWithScope');
    expect(evaluator).toContain('evaluateAstOwnedWithScope: async');
  });
});
