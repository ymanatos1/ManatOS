...
describe('metadata hierarchy workspace', () => {
  it('discovers a reusable self-reference contract from metadata and keeps record identity as the collection key', () => {
    const descriptor = metadataHierarchyWorkspaceDescriptor(sysBOPrincipalsMetadata, sysBOPrincipalsUIMetadata);
  ...
  it('uses a populated calculated root field to identify the organization and treats lone Person/System roots as complete only when standalone', () => {
    const descriptor = metadataHierarchyWorkspaceDescriptor(sysBOPrincipalsMetadata, sysBOPrincipalsUIMetadata)!;
    expect(hierarchyRootIdForMember(
      { id: 'member', rootPrincipalId: 'root' },
      descriptor,
    )).toBe('root');
    expect(hierarchyRootIdForMember(
      { id: 'root', rootPrincipalId: null },
      descriptor,
    )).toBe('root');

    expect(hierarchyFinalizationState([
      { id: 'person', parentId: null, principalType: SysBOPrincipalType.Person },
    ], sysBOPrincipalsMetadata, descriptor).complete).toBe(true);
    ...
    ], sysBOPrincipalsMetadata, descriptor);
