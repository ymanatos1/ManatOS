...
/**
 * Project entry-representation metadata for browser consumers. Authored source
 * remains the only executable-expression representation crossing this boundary.
 * AST ownership stays outside semantic/presentation state.
 */
  ...
  return {
    ...
    calculations: Object.fromEntries(
      Object.entries(metadata.fieldDefinition)
        ...
        .map(([key, field]) => [key, { source: field.calculation!.expression }]),
    ),
