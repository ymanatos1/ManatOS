...
export function entryNameSource<T>(metadata: ManatOSObjectMetadata<T>): ManatOSEntryValueSourceMetadata {
  return metadata.entry?.name
...
export function entryTypeSource<T>(metadata: ManatOSObjectMetadata<T>): ManatOSEntryValueSourceMetadata | null {
...
function sourceExpression(source: ManatOSEntryValueSourceMetadata): string {
  return 'expression' in source ? source.expression : source.field;
}
...
function directSourceField<T>(metadata: ManatOSObjectMetadata<T>, source: ManatOSEntryValueSourceMetadata | null): string | null {
  if (!source) return null;
  if ('field' in source) return source.field;
  const candidate = source.expression.trim();
  return /^[A-Za-z_$][A-Za-z0-9_$]*$/.test(candidate) && metadata.fieldDefinition[candidate] ? candidate : null;
}
...
function matchingReferenceRow(
  ...
  return referenceData[fieldKey]?.find((row) => String(row.id ?? row.value ?? '') === String(value)) ?? null;
}
...
function resolvedIcons(
  ...
  if (mode === 'fixed' && iconMetadata?.mode === 'fixed') return [normalizeIcon(iconMetadata.icon)].filter((value): value is string => Boolean(value));
  if (mode === 'type') return typeIcon ? [typeIcon] : [];
  if (mode === 'composed') return [entityIcon, typeIcon].filter((value): value is string => Boolean(value));
...
export function resolveEntryRepresentation<T>(
  ...
  const relations = options.relations ?? relationshipProjection(metadata, entry, options.referenceData);
  const nameSource = entryNameSource(metadata);
  const typeSource = entryTypeSource(metadata);
  const typeValue = typeSource ? resolveSource(metadata, entry, typeSource, relations, 'type') : null;
  ...
  const relationTypeRow = relationTypeKey && relations[relationTypeKey] && typeof relations[relationTypeKey] === 'object'
    ? relations[relationTypeKey] as Readonly<Record<string, unknown>>
    : null;
  const referenceRow = typeFieldKey && typeField?.type === 'reference'
  ...
  return {
    ...
    typeName,
    typeIcon,
    description: stringOrNull(resolveSource(metadata, entry, metadata.entry?.description, relations, 'description')),
